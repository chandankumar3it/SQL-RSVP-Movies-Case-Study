...
-- INSIGHT: Total rows available in each table
...
SELECT SUM(CASE
       ...
       SUM(CASE
             ...
             ELSE 0
           END) DATE_PUBLIESHED_NULL_COUNT,
       SUM(CASE
             WHEN duration IS NULL THEN 1
             ELSE 0
           END) DURATION_NULL_COUNT,
       SUM(CASE
             WHEN country IS NULL THEN 1
             ELSE 0
           END) COUNTRY_NULL_COUNT,
       SUM(CASE
             WHEN worlwide_gross_income IS NULL THEN 1
             ELSE 0
           END) W_G_I_NULL_COUNT,
       SUM(CASE
       ...
       SUM(CASE
             ...
             ELSE 0
           END) PROD_COMP_NULL_COUNT
FROM   movie; 
-- INSIGHTS: These columns containing null records: country - 20, worlwide_gross_income - 3724, languages - 194, production_company - 528
...
-- INSIGHTS: Total movies produced in USA or Inda in year 2019 is - 1059
...
-- INSIGHT:- Here total Unique genres is 13
...
-- INSIGHT- Here Drama is produced 1078 movies in 2019 
...
-- INSIGHT- Here 3289 movies belongs to only 1 genre.
...
-- INSIGHT- Genre 'Thriller' has rank 3 with movie 1484.
...
-- INSIGHT- German movies received the highest number of votes 106710 when queried against country columns.
...
-- INSIGHTS - Height - 17335, Date of birth - 13431 and known for movies - 15226 having null values
		--  - There are no nulls in column 'name'
...
-- INSIGHTS - Here director James Mangold - 4, Anthony Russo - 3 and Soubin Shahir - 3 have movie
...
-- INSIGHTS- Actor Mammootty has completed 8 movies and Mohanlal has completed 5 movies.
...
-- INSIGHTS- 'Marvel Studios' is number 1 Production company whose got the higher votes.
...
-- INSIGHTS - 'Vijay Sethupathi' is top actor
...
-- INSIGHTS- 'Taapsee Pannu' have average rating 7.74 which top most.
...
WITH top_3_genre AS
( 	
	SELECT g.genre, COUNT(g.movie_id) AS number_of_movies
    FROM genre AS g
    INNER JOIN movie AS m
    ON g.movie_id = m.id
    GROUP BY g.genre
...
-- INSIGHTS- 'Star Cinema' and 'Twentieth Century Fox' are top two production houses that have produced the highest number of hits 
...
SELECT n.name, SUM(r.total_votes) AS total_votes, COUNT(rm.movie_id) AS movie_count, r.avg_rating,
...
(
	SELECT d.name_id, n.name, d.movie_id, m.duration, r.avg_rating, total_votes, m.date_published,
	LEAD(m.date_published,1) OVER (partition BY d.name_id ORDER BY m.date_published, movie_id ) AS next_date_published
    FROM director_mapping d
    INNER JOIN names n 
    ON n.id = d.name_id
    INNER JOIN movie m
